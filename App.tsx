...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Show a welcome popup on initial load.
    // The popup content is bilingual, so we use the content from the initial language `t`.
    const popupContent = t.welcomePopup;

    Swal.fire({
      title: `<span style="font-family: 'Sarabun', sans-serif;">${popupContent.title}</span>`,
      html: `<div style="font-family: 'Sarabun', sans-serif;">${popupContent.html}</div>`,
      icon: 'info',
      confirmButtonText: `<span style="font-family: 'Sarabun', sans-serif;">${popupContent.confirmButtonText}</span>`,
